...
### Backend template
- `src/<module>/<model>.gcl` - Data models + global indices
- `src/<module>/<module>_api.gcl` - `@expose` functions and associated `@volatile` types
- `src/<module>/<module>_reader.gcl` - Readers (CSV, JSON, Parquet, etc.)
- `src/<module>/<module>_writer.gcl` - Writers
- `test/<module>_test.gcl` - Tests

Small features: `src/<module>.gcl` — one file is fine.
...
**Fetching latest library versions**: `curl https://get.greycat.io/files/<lib>/stable/latest` — returns `<lib>/<ver>/<lib>-<ver>` (use the part after `/`). Exception: `std` uses `core` as the URL path (all others match: `ai`, `explorer`, `algebra`, etc.).
...
| `time` | `Map<time,V>` | `nodeTime<V>` |
| `geo` | `Map<geo,V>` | `nodeGeo<V>` |
...
**API** — return `Array<T>` with `@volatile`, never nodeList:
```gcl
@volatile
...
| `nodeList<T>` | `nodeList<node<T>>` for complex types |
